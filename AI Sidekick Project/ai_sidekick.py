...
class AI_Sidekick:
  ...
  def ai_evaluator(self, state:State)-> Dict[str, Any]:
   ...
   user_prompt = f"""
                ...
                Please note that the Assistant has access to a tool to write files, so if the assistant says that they have written a file, then you can be sure that they have done so.

                Thus overall you should give the assistant the benefit of the doubt if they have done something.

                However, you should still eventually make sure that the success criteria is met, as that is the main end goal, and let the assistant know if more work is required based on your evaluation or if the need for additional user inputs arises otherwise.
